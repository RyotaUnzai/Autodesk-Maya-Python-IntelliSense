...
def aaf2fcp(flagdeleteFile: boolean, flagdstPath: string, flaggetFileName: int, flagprogress: int, flagsrcFile: string, flagterminate: int, flagwaitCompletion: int) -> string:
    """Synopsis:
---
---
...
def about(flagapiVersion: boolean, flagapplication: boolean, flagarm64: boolean, flagbatch: boolean, flagbuildDirectory: boolean, flagbuildVariant: boolean, flagcodeset: boolean, flagcompositingManager: boolean, flagconnected: boolean, flagcreativeVersion: boolean, flagctime: boolean, flagcurrentDate: boolean, flagcurrentTime: boolean, flagcustomVersion: boolean, flagcustomVersionClient: boolean, flagcustomVersionMajor: boolean, flagcustomVersionMinor: boolean, flagcustomVersionString: boolean, flagcutIdentifier: boolean, flagdate: boolean, flagenvironmentFile: boolean, flagevalVersion: boolean, flagfile: boolean, flagfontInfo: boolean, flaghelpDataDirectory: boolean, flaginstalledVersion: boolean, flagioVersion: boolean, flagirix: boolean, flagis64: boolean, flaglanguageResources: boolean, flaglinux: boolean, flaglinux64: boolean, flagliveUpdate: boolean, flaglocalizedResourceLocation: boolean, flagltVersion: boolean, flagmacOS: boolean, flagmacOSASi: boolean, flagmacOSppc: boolean, flagmacOSx86: boolean, flagmajorVersion: boolean, flagminorVersion: boolean, flagntOS: boolean, flagoperatingSystem: boolean, flagoperatingSystemVersion: boolean, flagpatchVersion: boolean, flagpreferences: boolean, flagproduct: boolean, flagqtVersion: boolean, flagtablet: boolean, flagtabletMode: boolean, flaguiLanguage: boolean, flaguiLanguageForStartup: boolean, flaguiLanguageIsLocalized: boolean, flaguiLocaleLanguage: boolean, flagversion: boolean, flagwin64: boolean, flagwindowManager: boolean, flagwindows: boolean) -> string:
    """Synopsis:
---
---
...
def addAttr(flagattributeType: string, flagbinaryTag: string, flagcachedInternally: boolean, flagcategory: string, flagdataType: string, flagdefaultValue: float, flagdisconnectBehaviour: uint, flagenumName: string, flagexists: boolean, flagfromPlugin: boolean, flaghasMaxValue: boolean, flaghasMinValue: boolean, flaghasSoftMaxValue: boolean, flaghasSoftMinValue: boolean, flaghidden: boolean, flagindexMatters: boolean, flaginternalSet: boolean, flagkeyable: boolean, flaglongName: string, flagmaxValue: float, flagminValue: float, flagmulti: boolean, flagniceName: string, flagnumberOfChildren: uint, flagparent: string, flagproxy: string, flagreadable: boolean, flagshortName: string, flagsoftMaxValue: float, flagsoftMinValue: float, flagstorable: boolean, flagusedAsColor: boolean, flagusedAsFilename: boolean, flagusedAsProxy: boolean, flagworldSpace: boolean, flagwritable: boolean) -> None:
    """Synopsis:
---
---
...
def addExtension(flagattributeType: string, flagbinaryTag: string, flagcachedInternally: boolean, flagcategory: string, flagdataType: string, flagdefaultValue: float, flagdisconnectBehaviour: uint, flagenumName: string, flagexists: boolean, flagfromPlugin: boolean, flaghasMaxValue: boolean, flaghasMinValue: boolean, flaghasSoftMaxValue: boolean, flaghasSoftMinValue: boolean, flaghidden: boolean, flagindexMatters: boolean, flaginternalSet: boolean, flagkeyable: boolean, flaglongName: string, flagmaxValue: float, flagminValue: float, flagmulti: boolean, flagniceName: string, flagnodeType: string, flagnumberOfChildren: uint, flagparent: string, flagproxy: string, flagreadable: boolean, flagshortName: string, flagsoftMaxValue: float, flagsoftMinValue: float, flagstorable: boolean, flagusedAsColor: boolean, flagusedAsFilename: boolean, flagusedAsProxy: boolean, flagworldSpace: boolean, flagwritable: boolean) -> None:
    """Synopsis:
---
---
...
def addMetadata(flagchannelName: string, flagchannelType: string, flagindexType: string, flagscene: boolean, flagstreamName: string, flagstructure: string) -> list[string]:
    """Synopsis:
---
---
...
def addPP(flagattribute: string) -> list[string]:
    """Synopsis:
---
---
 addPP(
...
def affectedNet(flagtype: string) -> None:
    """Synopsis:
---
---
 affectedNet(
...
def affects(flagby: boolean, flagtype: string) -> string:
    """Synopsis:
---
---
...
def aimConstraint(flagaimVector: tuple[float, float, float], flaglayer: string, flagmaintainOffset: boolean, flagname: string, flagoffset: tuple[float, float, float], flagremove: boolean, flagskip: string, flagtargetList: boolean, flagupVector: tuple[float, float, float], flagweight: float, flagweightAliasList: boolean, flagworldUpObject: name, flagworldUpType: string, flagworldUpVector: tuple[float, float, float]) -> list[string]:
    """Synopsis:
---
---
 aimConstraint(
...
def air(flagattenuation: float, flagdirectionX: float, flagdirectionY: float, flagdirectionZ: float, flagenableSpread: boolean, flagfanSetup: boolean, flaginheritRotation: boolean, flaginheritVelocity: float, flagmagnitude: float, flagmaxDistance: linear, flagname: string, flagperVertex: boolean, flagposition: tuple[linear, linear, linear], flagspeed: float, flagspread: float, flagtorusSectionRadius: linear, flagvelocityComponentOnly: boolean, flagvolumeExclusion: boolean, flagvolumeOffset: tuple[linear, linear, linear], flagvolumeShape: string, flagvolumeSweep: angle, flagwakeSetup: boolean, flagwindSetup: boolean) -> string:
    """Synopsis:
---
---
 air(
...
def aliasAttr(flagremove: boolean) -> list[string]:
    """Synopsis:
---
---
...
def align(flagalignToLead: boolean, flagcoordinateSystem: name, flagxAxis: string, flagyAxis: string, flagzAxis: string) -> boolean:
    """Synopsis:
---
---
...
def alignCtx(flagalign: boolean, flaganchorFirstObject: boolean, flagdistribute: boolean, flagexists: boolean, flaghistory: boolean, flagimage1: string, flagimage2: string, flagimage3: string, flagname: string, flagshowAlignTouch: boolean) -> string:
    """Synopsis:
---
---
 alignCtx(
...
def alignCurve(flagattach: boolean, flagcaching: boolean, flagconstructionHistory: boolean, flagcurvatureContinuity: boolean, flagcurvatureScale1: float, flagcurvatureScale2: float, flagjoinParameter: float, flagkeepMultipleKnots: boolean, flagname: string, flagnodeState: int, flagobject: boolean, flagpositionalContinuity: boolean, flagpositionalContinuityType: int, flagreplaceOriginal: boolean, flagreverse1: boolean, flagreverse2: boolean, flagtangentContinuity: boolean, flagtangentContinuityType: int, flagtangentScale1: float, flagtangentScale2: float) -> list[string]:
    """Synopsis:
---
---
 alignCurve(
...
def alignSurface(flagattach: boolean, flagcaching: boolean, flagconstructionHistory: boolean, flagcurvatureContinuity: boolean, flagcurvatureScale1: float, flagcurvatureScale2: float, flagdirectionU: boolean, flagjoinParameter: float, flagkeepMultipleKnots: boolean, flagname: string, flagnodeState: int, flagobject: boolean, flagpositionalContinuity: boolean, flagpositionalContinuityType: int, flagreplaceOriginal: boolean, flagreverse1: boolean, flagreverse2: boolean, flagswap1: boolean, flagswap2: boolean, flagtangentContinuity: boolean, flagtangentContinuityType: int, flagtangentScale1: float, flagtangentScale2: float, flagtwist: boolean) -> list[string]:
    """Synopsis:
---
---
 alignSurface(
...
def allNodeTypes(flagincludeAbstract: boolean) -> list[string]:
    """Synopsis:
---
---
...
def angleBetween(flagcaching: boolean, flagconstructionHistory: boolean, flageuler: boolean, flagnodeState: int, flagvector1: tuple[linear, linear, linear], flagvector1X: linear, flagvector1Y: linear, flagvector1Z: linear, flagvector2: tuple[linear, linear, linear], flagvector2X: linear, flagvector2Y: linear, flagvector2Z: linear) -> float[] | string:
    """Synopsis:
---
---
...
def animCurveEditor(flagareCurvesSelected: boolean, flagautoFit: string, flagautoFitTime: string, flagclassicMode: boolean, flagclipTime: string, flagconstrainDrag: uint, flagcontrol: boolean, flagcurvesShown: boolean, flagcurvesShownForceUpdate: boolean, flagdefineTemplate: string, flagdenormalizeCurvesCommand: string, flagdisplayActiveKeyTangents: string, flagdisplayActiveKeys: string, flagdisplayInfinities: string, flagdisplayKeys: string, flagdisplayNormalized: boolean, flagdisplayTangents: string, flagdisplayValues: string, flagdocTag: string, flagexists: boolean, flagfilter: string, flagforceMainConnection: string, flaghighlightAffectedCurves: boolean, flaghighlightConnection: string, flagkeyMinScale: float, flagkeyScale: float, flagkeyingTime: string, flaglimitToSelectedCurves: boolean, flaglockMainConnection: boolean, flaglockPlayRangeShades: string, flaglookAt: string, flagmainListConnection: string, flagmenu: script, flagnormalizeCurvesCommand: string, flagoutliner: string, flagpanel: string, flagparent: string, flagpreSelectionHighlight: boolean, flagrenormalizeCurves: boolean, flagresultSamples: time, flagresultScreenSamples: int, flagresultUpdate: string, flagselectionConnection: string, flagshowActiveCurveNames: boolean, flagshowBufferCurves: string, flagshowCurveNames: boolean, flagshowPlayRangeShades: string, flagshowResults: string, flagshowUpstreamCurves: boolean, flagsimpleKeyView: boolean, flagsmoothness: string, flagsnapTime: string, flagsnapValue: string, flagstackedCurves: boolean, flagstackedCurvesMax: float, flagstackedCurvesMin: float, flagstackedCurvesSpace: float, flagstateString: boolean, flagtimelinePositionTop: boolean, flagunParent: boolean, flagunlockMainConnection: boolean, flagupdateMainConnection: boolean, flaguseTemplate: string, flagvalueLinesToggle: string) -> string:
    """Synopsis:
---
---
 animCurveEditor(
...
def animDisplay(flagmodelUpdate: string, flagrefAnimCurvesEditable: boolean, flagtimeCode: string, flagtimeCodeOffset: string) -> None:
    """Synopsis:
---
---
...
def animLayer(flagaddRelatedKG: boolean, flagaddSelectedObjects: boolean, flagaffectedLayers: boolean, flaganimCurves: boolean, flagattribute: string, flagbaseAnimCurves: boolean, flagbestAnimLayer: boolean, flagbestLayer: boolean, flagblendNodes: boolean, flagchildren: string, flagcollapse: boolean, flagcopy: string, flagcopyAnimation: string, flagcopyNoAnimation: string, flagexcludeBoolean: boolean, flagexcludeDynamic: boolean, flagexcludeEnum: boolean, flagexcludeProxy: boolean, flagexcludeRotate: boolean, flagexcludeScale: boolean, flagexcludeTranslate: boolean, flagexcludeVisibility: boolean, flagexists: boolean, flagextractAnimation: string, flagfindCurveForPlug: string, flagforceUIRebuild: boolean, flagforceUIRefresh: boolean, flaglayeredPlug: string, flaglock: boolean, flagmaxLayers: boolean, flagmoveLayerAfter: string, flagmoveLayerBefore: string, flagmute: boolean, flagoverride: boolean, flagparent: string, flagpassthrough: boolean, flagpreferred: boolean, flagremoveAllAttributes: boolean, flagremoveAttribute: string, flagremoveSelectedObjects: boolean, flagroot: string, flagselected: boolean, flagsolo: boolean, flagweight: float, flagwriteBlendnodeDestinations: boolean) -> string:
    """Synopsis:
---
---
...
def animView(flagendTime: time, flagmaxValue: float, flagminValue: float, flagnextView: boolean, flagpreviousView: boolean, flagstartTime: time) -> None:
    """Synopsis:
---
---
 animView(
...
def annotate(flagpoint: tuple[linear, linear, linear], flagtext: string) -> string:
    """Synopsis:
---
---
 annotate(
...
def appHome(flagiconVisible: boolean, flaginstrument: string, flagsetTab: string, flagtoggleVisibility: boolean, flagupdateRecentFiles: boolean, flagvisible: boolean) -> None:
    """Synopsis:
---
---
...
def applyAttrPattern(flagnodeType: string, flagpatternName: string) -> int:
    """Synopsis:
---
---
...
def applyMetadata(flagformat: string, flagscene: boolean, flagvalue: string) -> Boolean:
    """Synopsis:
---
---
...
def applyTake(flagchannel: string, flagdevice: string, flagfilter: string, flagpreview: boolean, flagrecurseChannel: boolean, flagreset: boolean, flagspecifyChannel: boolean, flagstartTime: time) -> None:
    """Synopsis:
---
---
...
def arcLenDimContext(flagexists: boolean, flaghistory: boolean, flagimage1: string, flagimage2: string, flagimage3: string, flagname: string) -> string:
    """Synopsis:
---
---
...
def arclen(flagcaching: boolean, flagconstructionHistory: boolean, flagnodeState: int) -> float | string:
    """Synopsis:
---
---
 arclen(
...
def arrayMapper(flagdestAttr: string, flaginputU: string, flaginputV: string, flagmapTo: string, flagtarget: string, flagtype: string) -> list[string]:
    """Synopsis:
---
---
...
def art3dPaintCtx(flagaccopacity: boolean, flagafterStrokeCmd: string, flagalphablendmode: string, flagassigntxt: boolean, flagattrnames: string, flagbeforeStrokeCmd: string, flagbrushalignment: boolean, flagbrushdepth: float, flagbrushfeedback: boolean, flagbrushtype: string, flagclear: boolean, flagcommonattr: string, flagdragSlider: string, flagdynclonemode: boolean, flagexists: boolean, flagexpandfilename: boolean, flagextendFillColor: boolean, flagfileformat: string, flagfiletxtaspectratio: float, flagfiletxtsizex: int, flagfiletxtsizey: int, flagfloodOpacity: float, flagfloodall: boolean, flagfloodselect: boolean, flaghistory: boolean, flagimage1: string, flagimage2: string, flagimage3: string, flagkeepaspectratio: boolean, flaglastRecorderCmd: string, flaglastStampName: string, flaglowerradius: float, flagmakeStroke: uint, flagmappressure: string, flagname: string, flagopacity: float, flagoutline: boolean, flagoutwhilepaint: boolean, flagpaintmode: string, flagpaintoperationtype: string, flagpainttxtattr: string, flagpainttxtattrname: string, flagpfxScale: float, flagpfxWidth: float, flagpickColor: boolean, flagpickValue: boolean, flagplaybackCursor: tuple[float, float], flagplaybackPressure: float, flagpreserveclonesource: boolean, flagpressureMapping1: int, flagpressureMapping2: int, flagpressureMapping3: int, flagpressureMax1: float, flagpressureMax2: float, flagpressureMax3: float, flagpressureMin1: float, flagpressureMin2: float, flagpressureMin3: float, flagprofileShapeFile: string, flagprojective: boolean, flagradius: float, flagrecord: boolean, flagreflection: boolean, flagreflectionaboutorigin: boolean, flagreflectionaxis: string, flagreloadtexfile: boolean, flagresizeratio: float, flagresizetxt: boolean, flagrgbcolor: tuple[float, float, float], flagrgbflood: tuple[float, float, float], flagsaveTextureOnStroke: boolean, flagsaveonstroke: boolean, flagsavetexture: boolean, flagscreenRadius: float, flagselectclonesource: boolean, flagshadernames: string, flagshapeattr: boolean, flagshapenames: string, flagshowactive: boolean, flagsoloAsDiffuse: boolean, flagstampDepth: float, flagstampProfile: string, flagstampSpacing: float, flagstrokesmooth: string, flagsurfaceConformedBrushVertices: boolean, flagtablet: boolean, flagtangentOutline: boolean, flagtextureFilenames: boolean, flagupdateEraseTex: boolean, flagusepressure: boolean, flagworldRadius: float) -> None:
    """Synopsis:
---
---
...
def artAttrCtx(flagaccopacity: boolean, flagactiveListChangedProc: string, flagafterStrokeCmd: string, flagalphaclamp: string, flagalphaclamplower: float, flagalphaclampupper: float, flagattrSelected: string, flagbeforeStrokeCmd: string, flagbrushalignment: boolean, flagbrushfeedback: boolean, flagclamp: string, flagclamplower: float, flagclampupper: float, flagclear: boolean, flagcolorAlphaValue: float, flagcolorRGBAValue: tuple[float, float, float, float], flagcolorRGBValue: tuple[float, float, float], flagcolorRamp: string, flagcolorfeedback: boolean, flagcolorfeedbackOverride: boolean, flagcolorrangelower: float, flagcolorrangeupper: float, flagdataTypeIndex: int, flagdisablelighting: boolean, flagdragSlider: string, flagduringStrokeCmd: string, flagdynclonemode: boolean, flagexists: boolean, flagexpandfilename: boolean, flagexportaspectratio: float, flagexportfilemode: string, flagexportfilesave: string, flagexportfilesizex: int, flagexportfilesizey: int, flagexportfiletype: string, flagfilterNodes: boolean, flaghistory: boolean, flagimage1: string, flagimage2: string, flagimage3: string, flagimportfileload: string, flagimportfilemode: string, flagimportreassign: boolean, flaginteractiveUpdate: boolean, flaglastRecorderCmd: string, flaglastStampName: string, flaglowerradius: float, flagmakeStroke: uint, flagmappressure: string, flagmaxvalue: float, flagminvalue: float, flagname: string, flagnumericColorRamp: string, flagnumericDisplayColor: tuple[float, float, float], flagnumericDisplayPrecision: uint, flagnumericMaxColor: tuple[float, float, float], flagnumericMinColor: tuple[float, float, float], flagobjattrArray: string, flagobjattrArrayNoMenu: string, flagopacity: float, flagoutline: boolean, flagoutwhilepaint: boolean, flagpaintNodeArray: string, flagpaintattrselected: string, flagpaintmode: string, flagpaintoperationtype: string, flagpickColor: boolean, flagpickValue: boolean, flagplaybackCursor: tuple[float, float], flagplaybackPressure: float, flagpreserveclonesource: boolean, flagprofileShapeFile: string, flagprojective: boolean, flagradius: float, flagrampMaxColor: tuple[float, float, float], flagrampMinColor: tuple[float, float, float], flagrecord: boolean, flagreflection: boolean, flagreflectionaboutorigin: boolean, flagreflectionaxis: string, flagscreenRadius: float, flagselectclonesource: boolean, flagselectedattroper: string, flagshowactive: boolean, flagstampDepth: float, flagstampProfile: string, flagstampSpacing: float, flagstrokesmooth: string, flagsurfaceConformedBrushVertices: boolean, flagtablet: boolean, flagtangentOutline: boolean, flagtoolOffProc: string, flagtoolOnProc: string, flaguseColorRamp: boolean, flaguseMaxMinColor: boolean, flaguseNumericColorRamp: boolean, flaguseNumericDisplay: boolean, flagusepressure: boolean, flagvalue: float, flagwhichTool: string, flagworldRadius: float) -> string:
    """Synopsis:
---
---
...
def artAttrPaintVertexCtx(flagaccopacity: boolean, flagactiveListChangedProc: string, flagafterStrokeCmd: string, flagalphaclamp: string, flagalphaclamplower: float, flagalphaclampupper: float, flagattrSelected: string, flagbeforeStrokeCmd: string, flagbrushalignment: boolean, flagbrushfeedback: boolean, flagclamp: string, flagclamplower: float, flagclampupper: float, flagclear: boolean, flagcolorAlphaValue: float, flagcolorRGBAValue: tuple[float, float, float, float], flagcolorRGBValue: tuple[float, float, float], flagcolorRamp: string, flagcolorfeedback: boolean, flagcolorfeedbackOverride: boolean, flagcolorrangelower: float, flagcolorrangeupper: float, flagdataTypeIndex: int, flagdisablelighting: boolean, flagdragSlider: string, flagduringStrokeCmd: string, flagdynclonemode: boolean, flagexists: boolean, flagexpandfilename: boolean, flagexportaspectratio: float, flagexportfilemode: string, flagexportfilesave: string, flagexportfilesizex: int, flagexportfilesizey: int, flagexportfiletype: string, flagfilterNodes: boolean, flaghistory: boolean, flagimage1: string, flagimage2: string, flagimage3: string, flagimportfileload: string, flagimportfilemode: string, flagimportreassign: boolean, flaginteractiveUpdate: boolean, flaglastRecorderCmd: string, flaglastStampName: string, flaglowerradius: float, flagmakeStroke: uint, flagmappressure: string, flagmaxvalue: float, flagminvalue: float, flagname: string, flagnumericColorRamp: string, flagnumericDisplayColor: tuple[float, float, float], flagnumericDisplayPrecision: uint, flagnumericMaxColor: tuple[float, float, float], flagnumericMinColor: tuple[float, float, float], flagobjattrArray: string, flagobjattrArrayNoMenu: string, flagopacity: float, flagoutline: boolean, flagoutwhilepaint: boolean, flagpaintChannel: string, flagpaintComponent: int, flagpaintNodeArray: string, flagpaintNumChannels: int, flagpaintRGBA: boolean, flagpaintVertexFace: boolean, flagpaintattrselected: string, flagpaintmode: string, flagpaintoperationtype: string, flagpickColor: boolean, flagpickValue: boolean, flagplaybackCursor: tuple[float, float], flagplaybackPressure: float, flagpreserveclonesource: boolean, flagprofileShapeFile: string, flagprojective: boolean, flagradius: float, flagrampMaxColor: tuple[float, float, float], flagrampMinColor: tuple[float, float, float], flagrecord: boolean, flagreflection: boolean, flagreflectionaboutorigin: boolean, flagreflectionaxis: string, flagscreenRadius: float, flagselectclonesource: boolean, flagselectedattroper: string, flagshowactive: boolean, flagstampDepth: float, flagstampProfile: string, flagstampSpacing: float, flagstrokesmooth: string, flagsurfaceConformedBrushVertices: boolean, flagtablet: boolean, flagtangentOutline: boolean, flagtoolOffProc: string, flagtoolOnProc: string, flaguseColorRamp: boolean, flaguseMaxMinColor: boolean, flaguseNumericColorRamp: boolean, flaguseNumericDisplay: boolean, flagusepressure: boolean, flagvalue: float, flagvertexColorRange: boolean, flagvertexColorRangeLower: float, flagvertexColorRangeUpper: float, flagwhichTool: string, flagworldRadius: float) -> None:
    """Synopsis:
---
---
...
def artAttrSkinPaintCtx(flagaccopacity: boolean, flagactiveListChangedProc: string, flagafterStrokeCmd: string, flagalphaclamp: string, flagalphaclamplower: float, flagalphaclampupper: float, flagattrSelected: string, flagbeforeStrokeCmd: string, flagbrushalignment: boolean, flagbrushfeedback: boolean, flagclamp: string, flagclamplower: float, flagclampupper: float, flagclear: boolean, flagcolorAlphaValue: float, flagcolorRGBAValue: tuple[float, float, float, float], flagcolorRGBValue: tuple[float, float, float], flagcolorRamp: string, flagcolorfeedback: boolean, flagcolorfeedbackOverride: boolean, flagcolorrangelower: float, flagcolorrangeupper: float, flagdataTypeIndex: int, flagdisablelighting: boolean, flagdragSlider: string, flagduringStrokeCmd: string, flagdynclonemode: boolean, flagexists: boolean, flagexpandfilename: boolean, flagexportaspectratio: float, flagexportfilemode: string, flagexportfilesave: string, flagexportfilesizex: int, flagexportfilesizey: int, flagexportfiletype: string, flagfilterNodes: boolean, flaghistory: boolean, flagimage1: string, flagimage2: string, flagimage3: string, flagimportfileload: string, flagimportfilemode: string, flagimportreassign: boolean, flaginfluence: string, flaginteractiveUpdate: boolean, flaglastRecorderCmd: string, flaglastStampName: string, flaglowerradius: float, flagmakeStroke: uint, flagmappressure: string, flagmaxvalue: float, flagminvalue: float, flagname: string, flagnumericColorRamp: string, flagnumericDisplayColor: tuple[float, float, float], flagnumericDisplayPrecision: uint, flagnumericMaxColor: tuple[float, float, float], flagnumericMinColor: tuple[float, float, float], flagobjattrArray: string, flagobjattrArrayNoMenu: string, flagopacity: float, flagoutline: boolean, flagoutwhilepaint: boolean, flagpaintNodeArray: string, flagpaintSelectMode: int, flagpaintattrselected: string, flagpaintmode: string, flagpaintoperationtype: string, flagpickColor: boolean, flagpickValue: boolean, flagplaybackCursor: tuple[float, float], flagplaybackPressure: float, flagpreserveclonesource: boolean, flagprofileShapeFile: string, flagprojective: boolean, flagradius: float, flagrampMaxColor: tuple[float, float, float], flagrampMinColor: tuple[float, float, float], flagrecord: boolean, flagreflection: boolean, flagreflectionaboutorigin: boolean, flagreflectionaxis: string, flagscreenRadius: float, flagselectclonesource: boolean, flagselectedattroper: string, flagshowactive: boolean, flagskinPaintMode: int, flagstampDepth: float, flagstampProfile: string, flagstampSpacing: float, flagstrokesmooth: string, flagsurfaceConformedBrushVertices: boolean, flagtablet: boolean, flagtangentOutline: boolean, flagtoolOffProc: string, flagtoolOnProc: string, flaguseColorRamp: boolean, flaguseMaxMinColor: boolean, flaguseNumericColorRamp: boolean, flaguseNumericDisplay: boolean, flagusepressure: boolean, flagvalue: float, flagwhichTool: string, flagworldRadius: float, flagxrayJoints: boolean) -> None:
    """Synopsis:
---
---
 artAttrSkinPaintCtx(
...
def artAttrTool(flagadd: string, flagexists: string, flagremove: string) -> None:
    """Synopsis:
---
---
...
def artFluidAttrCtx(flagaccopacity: boolean, flagactiveListChangedProc: string, flagafterStrokeCmd: string, flagalphaclamp: string, flagalphaclamplower: float, flagalphaclampupper: float, flagattrSelected: string, flagautoSave: string, flagbeforeStrokeCmd: string, flagbrushalignment: boolean, flagbrushfeedback: boolean, flagclamp: string, flagclamplower: float, flagclampupper: float, flagclear: boolean, flagcolorAlphaValue: float, flagcolorRGBAValue: tuple[float, float, float, float], flagcolorRGBValue: tuple[float, float, float], flagcolorRamp: string, flagcolorfeedback: boolean, flagcolorfeedbackOverride: boolean, flagcolorrangelower: float, flagcolorrangeupper: float, flagcurrentPaintableFluid: string, flagdataTypeIndex: int, flagdelaySelectionChanged: boolean, flagdisablelighting: boolean, flagdisplayAsRender: boolean, flagdisplayVelocity: boolean, flagdoAutoSave: boolean, flagdragSlider: string, flagduringStrokeCmd: string, flagdynclonemode: boolean, flagexists: boolean, flagexpandfilename: boolean, flagexportaspectratio: float, flagexportfilemode: string, flagexportfilesave: string, flagexportfilesizex: int, flagexportfilesizey: int, flagexportfiletype: string, flagfilterNodes: boolean, flaghistory: boolean, flagimage1: string, flagimage2: string, flagimage3: string, flagimportfileload: string, flagimportfilemode: string, flagimportreassign: boolean, flaginteractiveUpdate: boolean, flaglastRecorderCmd: string, flaglastStampName: string, flaglowerradius: float, flagmakeStroke: uint, flagmappressure: string, flagmaxvalue: float, flagminvalue: float, flagname: string, flagnumericColorRamp: string, flagnumericDisplayColor: tuple[float, float, float], flagnumericDisplayPrecision: uint, flagnumericMaxColor: tuple[float, float, float], flagnumericMinColor: tuple[float, float, float], flagobjattrArray: string, flagobjattrArrayNoMenu: string, flagopacity: float, flagoutline: boolean, flagoutwhilepaint: boolean, flagpaintNodeArray: string, flagpaintattrselected: string, flagpaintmode: string, flagpaintoperationtype: string, flagpickColor: boolean, flagpickValue: boolean, flagplaybackCursor: tuple[float, float], flagplaybackPressure: float, flagpreserveclonesource: boolean, flagprofileShapeFile: string, flagprojective: boolean, flagproperty: string, flagradius: float, flagrampMaxColor: tuple[float, float, float], flagrampMinColor: tuple[float, float, float], flagrecord: boolean, flagreflection: boolean, flagreflectionaboutorigin: boolean, flagreflectionaxis: string, flagrgbValue: tuple[float, float, float], flagscreenRadius: float, flagselectclonesource: boolean, flagselectedattroper: string, flagshowactive: boolean, flagstampDepth: float, flagstampProfile: string, flagstampSpacing: float, flagstrokesmooth: string, flagsurfaceConformedBrushVertices: boolean, flagtablet: boolean, flagtangentOutline: boolean, flagtoolOffProc: string, flagtoolOnProc: string, flaguseColorRamp: boolean, flaguseMaxMinColor: boolean, flaguseNumericColorRamp: boolean, flaguseNumericDisplay: boolean, flaguseStrokeDirection: boolean, flagusepressure: boolean, flagvalue: float, flagvelocity: tuple[float, float, float], flagwhichTool: string, flagworldRadius: float) -> None:
    """Synopsis:
---
---
...
def artPuttyCtx(flagaccopacity: boolean, flagactiveListChangedProc: string, flagafterStrokeCmd: string, flagalphaclamp: string, flagalphaclamplower: float, flagalphaclampupper: float, flagattrSelected: string, flagautosmooth: boolean, flagbeforeStrokeCmd: string, flagbrushStrength: float, flagbrushalignment: boolean, flagbrushfeedback: boolean, flagclamp: string, flagclamplower: float, flagclampupper: float, flagclear: boolean, flagcollapsecvtol: float, flagcolorAlphaValue: float, flagcolorRGBAValue: tuple[float, float, float, float], flagcolorRGBValue: tuple[float, float, float], flagcolorRamp: string, flagcolorfeedback: boolean, flagcolorfeedbackOverride: boolean, flagcolorrangelower: float, flagcolorrangeupper: float, flagdataTypeIndex: int, flagdisablelighting: boolean, flagdispdecr: boolean, flagdispincr: boolean, flagdragSlider: string, flagduringStrokeCmd: string, flagdynclonemode: boolean, flagerasesrfupd: boolean, flagexists: boolean, flagexpandfilename: boolean, flagexportaspectratio: float, flagexportfilemode: string, flagexportfilesave: string, flagexportfilesizex: int, flagexportfilesizey: int, flagexportfiletype: string, flagfilterNodes: boolean, flaghistory: boolean, flagimage1: string, flagimage2: string, flagimage3: string, flagimportfileload: string, flagimportfilemode: string, flagimportreassign: boolean, flaginteractiveUpdate: boolean, flaginvertrefvector: boolean, flaglastRecorderCmd: string, flaglastStampName: string, flaglowerradius: float, flagmakeStroke: uint, flagmappressure: string, flagmaxdisp: float, flagmaxvalue: float, flagminvalue: float, flagmouldtypehead: string, flagmouldtypemouse: string, flagmouldtypetail: string, flagname: string, flagnumericColorRamp: string, flagnumericDisplayColor: tuple[float, float, float], flagnumericDisplayPrecision: uint, flagnumericMaxColor: tuple[float, float, float], flagnumericMinColor: tuple[float, float, float], flagobjattrArray: string, flagobjattrArrayNoMenu: string, flagopacity: float, flagoutline: boolean, flagoutwhilepaint: boolean, flagpaintNodeArray: string, flagpaintattrselected: string, flagpaintmode: string, flagpaintoperationtype: string, flagpickColor: boolean, flagpickValue: boolean, flagplaybackCursor: tuple[float, float], flagplaybackPressure: float, flagpolecv: boolean, flagpreserveclonesource: boolean, flagprofileShapeFile: string, flagprojective: boolean, flagradius: float, flagrampMaxColor: tuple[float, float, float], flagrampMinColor: tuple[float, float, float], flagrecord: boolean, flagreflection: boolean, flagreflectionaboutorigin: boolean, flagreflectionaxis: string, flagrefsurface: boolean, flagrefvector: string, flagrefvectoru: float, flagrefvectorv: float, flagscreenRadius: float, flagselectclonesource: boolean, flagselectedattroper: string, flagshowactive: boolean, flagsmoothiters: int, flagstampDepth: float, flagstampProfile: string, flagstampSpacing: float, flagstitchcorner: boolean, flagstitchedgeflood: boolean, flagstitchtype: string, flagstrokesmooth: string, flagsurfaceConformedBrushVertices: boolean, flagtablet: boolean, flagtangentOutline: boolean, flagtoolOffProc: string, flagtoolOnProc: string, flagupdateerasesrf: boolean, flagupdaterefsrf: boolean, flaguseColorRamp: boolean, flaguseMaxMinColor: boolean, flaguseNumericColorRamp: boolean, flaguseNumericDisplay: boolean, flagusepressure: boolean, flagvalue: float, flagwhichTool: string, flagworldRadius: float) -> None:
    """Synopsis:
---
---
...
def artSelectCtx(flagaccopacity: boolean, flagaddselection: boolean, flagafterStrokeCmd: string, flagbeforeStrokeCmd: string, flagbrushalignment: boolean, flagbrushfeedback: boolean, flagclear: boolean, flagdragSlider: string, flagdynclonemode: boolean, flagexists: boolean, flagexpandfilename: boolean, flagexportaspectratio: float, flagexportfilemode: string, flagexportfilesave: string, flagexportfilesizex: int, flagexportfilesizey: int, flagexportfiletype: string, flaghistory: boolean, flagimage1: string, flagimage2: string, flagimage3: string, flagimportfileload: string, flagimportfilemode: string, flagimportreassign: boolean, flagimportthreshold: float, flaglastRecorderCmd: string, flaglastStampName: string, flaglowerradius: float, flagmakeStroke: uint, flagmappressure: string, flagname: string, flagopacity: float, flagoutline: boolean, flagoutwhilepaint: boolean, flagpaintmode: string, flagpaintoperationtype: string, flagpickColor: boolean, flagpickValue: boolean, flagplaybackCursor: tuple[float, float], flagplaybackPressure: float, flagpreserveclonesource: boolean, flagprofileShapeFile: string, flagprojective: boolean, flagradius: float, flagrecord: boolean, flagreflection: boolean, flagreflectionaboutorigin: boolean, flagreflectionaxis: string, flagscreenRadius: float, flagselectall: boolean, flagselectclonesource: boolean, flagselectop: string, flagshowactive: boolean, flagstampDepth: float, flagstampProfile: string, flagstampSpacing: float, flagstrokesmooth: string, flagsurfaceConformedBrushVertices: boolean, flagtablet: boolean, flagtangentOutline: boolean, flagtoggleall: boolean, flagunselectall: boolean, flagusepressure: boolean, flagworldRadius: float) -> None:
    """Synopsis:
---
---
...
def artSetPaintCtx(flagaccopacity: boolean, flagafterStrokeCmd: string, flagbeforeStrokeCmd: string, flagbrushalignment: boolean, flagbrushfeedback: boolean, flagclear: boolean, flagdragSlider: string, flagdynclonemode: boolean, flagexists: boolean, flagexpandfilename: boolean, flagexportaspectratio: float, flagexportfilemode: string, flagexportfilesave: string, flagexportfilesizex: int, flagexportfilesizey: int, flagexportfiletype: string, flaghistory: boolean, flagimage1: string, flagimage2: string, flagimage3: string, flagimportfileload: string, flagimportfilemode: string, flagimportreassign: boolean, flaglastRecorderCmd: string, flaglastStampName: string, flaglowerradius: float, flagmakeStroke: uint, flagmappressure: string, flagname: string, flagobjectsetnames: string, flagopacity: float, flagoutline: boolean, flagoutwhilepaint: boolean, flagpaintmode: string, flagpaintoperationtype: string, flagpickColor: boolean, flagpickValue: boolean, flagplaybackCursor: tuple[float, float], flagplaybackPressure: float, flagpreserveclonesource: boolean, flagprofileShapeFile: string, flagprojective: boolean, flagradius: float, flagrecord: boolean, flagreflection: boolean, flagreflectionaboutorigin: boolean, flagreflectionaxis: string, flagscreenRadius: float, flagselectclonesource: boolean, flagsetcolorfeedback: boolean, flagsetdisplaycvs: boolean, flagsetopertype: string, flagsettomodify: string, flagshowactive: boolean, flagstampDepth: float, flagstampProfile: string, flagstampSpacing: float, flagstrokesmooth: string, flagsurfaceConformedBrushVertices: boolean, flagtablet: boolean, flagtangentOutline: boolean, flagusepressure: boolean, flagworldRadius: float) -> None:
    """Synopsis:
---
---
...
def artUserPaintCtx(flagaccopacity: boolean, flagactiveListChangedProc: string, flagafterStrokeCmd: string, flagalphaclamp: string, flagalphaclamplower: float, flagalphaclampupper: float, flagattrSelected: string, flagbeforeStrokeCmd: string, flagbrushalignment: boolean, flagbrushfeedback: boolean, flagchunkCommand: string, flagclamp: string, flagclamplower: float, flagclampupper: float, flagclear: boolean, flagcolorAlphaValue: float, flagcolorRGBAValue: tuple[float, float, float, float], flagcolorRGBValue: tuple[float, float, float], flagcolorRamp: string, flagcolorfeedback: boolean, flagcolorfeedbackOverride: boolean, flagcolorrangelower: float, flagcolorrangeupper: float, flagdataTypeIndex: int, flagdisablelighting: boolean, flagdragSlider: string, flagduringStrokeCmd: string, flagdynclonemode: boolean, flagexists: boolean, flagexpandfilename: boolean, flagexportaspectratio: float, flagexportfilemode: string, flagexportfilesave: string, flagexportfilesizex: int, flagexportfilesizey: int, flagexportfiletype: string, flagfilterNodes: boolean, flagfinalizeCmd: string, flagfullpaths: boolean, flaggetArrayAttrCommand: string, flaggetSurfaceCommand: string, flaggetValueCommand: string, flaghistory: boolean, flagimage1: string, flagimage2: string, flagimage3: string, flagimportfileload: string, flagimportfilemode: string, flagimportreassign: boolean, flaginitializeCmd: string, flaginteractiveUpdate: boolean, flaglastRecorderCmd: string, flaglastStampName: string, flaglowerradius: float, flagmakeStroke: uint, flagmappressure: string, flagmaxvalue: float, flagminvalue: float, flagname: string, flagnumericColorRamp: string, flagnumericDisplayColor: tuple[float, float, float], flagnumericDisplayPrecision: uint, flagnumericMaxColor: tuple[float, float, float], flagnumericMinColor: tuple[float, float, float], flagobjattrArray: string, flagobjattrArrayNoMenu: string, flagopacity: float, flagoutline: boolean, flagoutwhilepaint: boolean, flagpaintNodeArray: string, flagpaintattrselected: string, flagpaintmode: string, flagpaintoperationtype: string, flagpickColor: boolean, flagpickValue: boolean, flagplaybackCursor: tuple[float, float], flagplaybackPressure: float, flagpreserveclonesource: boolean, flagprofileShapeFile: string, flagprojective: boolean, flagradius: float, flagrampMaxColor: tuple[float, float, float], flagrampMinColor: tuple[float, float, float], flagrecord: boolean, flagreflection: boolean, flagreflectionaboutorigin: boolean, flagreflectionaxis: string, flagscreenRadius: float, flagselectclonesource: boolean, flagselectedattroper: string, flagsetArrayValueCommand: string, flagsetValueCommand: string, flagshowactive: boolean, flagstampDepth: float, flagstampProfile: string, flagstampSpacing: float, flagstrokesmooth: string, flagsurfaceConformedBrushVertices: boolean, flagtablet: boolean, flagtangentOutline: boolean, flagtoolCleanupCmd: string, flagtoolOffProc: string, flagtoolOnProc: string, flagtoolSetupCmd: string, flaguseColorRamp: boolean, flaguseMaxMinColor: boolean, flaguseNumericColorRamp: boolean, flaguseNumericDisplay: boolean, flagusepressure: boolean, flagvalue: float, flagwhichTool: string, flagworldRadius: float) -> string:
    """Synopsis:
---
---
...
def arubaNurbsToPoly(flagcaching: boolean, flagconstructionHistory: boolean, flaglocalSpace: boolean, flagname: string, flagnodeState: int, flagobject: boolean, flagworldSpace: boolean) -> list[string]:
    """Synopsis:
---
---
 arubaNurbsToPoly(
...
def assembly(flagactive: string, flagactiveLabel: string, flagcanCreate: string, flagcreateOptionBoxProc: script, flagcreateRepresentation: string, flagdefaultType: string, flagdeleteRepresentation: string, flagderegister: string, flaginput: string, flagisAType: string, flagisTrackingMemberEdits: string, flaglabel: string, flaglistRepTypes: boolean, flaglistRepTypesProc: script, flaglistRepresentations: boolean, flaglistTypes: boolean, flagname: string, flagnewRepLabel: string, flagpostCreateUIProc: script, flagproc: script, flagrenameRepresentation: string, flagrepLabel: string, flagrepName: string, flagrepNamespace: string, flagrepPostCreateUIProc: string, flagrepPreCreateUIProc: string, flagrepType: string, flagrepTypeLabel: string, flagrepTypeLabelProc: script, flagtype: string) -> None:
    """Synopsis:
---
---
...
def assignCommand(flagaddDivider: string, flagaltModifier: boolean, flagannotation: string, flagcommand: script, flagcommandModifier: boolean, flagctrlModifier: boolean, flagdata1: string, flagdata2: string, flagdata3: string, flagdelete: int, flagdividerString: string, flagenableCommandRepeat: boolean, flagfactorySettings: boolean, flagindex: int, flagkeyArray: boolean, flagkeyString: string, flagkeyUp: boolean, flagname: boolean, flagnumDividersPreceding: int, flagnumElements: boolean, flagoptionModifier: boolean, flagsortByKey: boolean, flagsourceUserCommands: boolean) -> None:
    """Synopsis:
---
---
 assignCommand(
...
def assignInputDevice(flagclutch: string, flagcontinuous: boolean, flagdevice: string, flagimmediate: boolean, flagmultiple: boolean) -> string:
    """Synopsis:
---
---
...
def assignViewportFactories(flagmaterialFactory: string, flagnodeType: string, flagtextureFactory: string) -> None:
    """Synopsis:
---
---
...
def attachCurve(flagblendBias: float, flagblendKnotInsertion: boolean, flagcaching: boolean, flagconstructionHistory: boolean, flagkeepMultipleKnots: boolean, flagmethod: int, flagname: string, flagnodeState: int, flagobject: boolean, flagparameter: float, flagreplaceOriginal: boolean, flagreverse1: boolean, flagreverse2: boolean) -> list[string]:
    """Synopsis:
---
---
 attachCurve(
...
def attachDeviceAttr(flagattribute: string, flagaxis: string, flagcamera: boolean, flagcameraRotate: boolean, flagcameraTranslate: boolean, flagclutch: string, flagdevice: string, flagselection: boolean) -> None:
    """Synopsis:
---
---
...
def attachSurface(flagblendBias: float, flagblendKnotInsertion: boolean, flagcaching: boolean, flagconstructionHistory: boolean, flagdirectionU: boolean, flagkeepMultipleKnots: boolean, flagmethod: int, flagname: string, flagnodeState: int, flagobject: boolean, flagparameter: float, flagreplaceOriginal: boolean, flagreverse1: boolean, flagreverse2: boolean, flagswap1: boolean, flagswap2: boolean, flagtwist: boolean) -> list[string]:
    """Synopsis:
---
---
 attachSurface(
...
def attrControlGrp(flagannotation: string, flagattribute: name, flagchangeCommand: script, flagenable: boolean, flagexists: boolean, flaghandlesAttribute: name, flaghideMapButton: boolean, flaglabel: string, flagpreventOverride: boolean) -> string:
    """Synopsis:
---
---
...
def attrEnumOptionMenuGrp(flagadjustableColumn: int, flagadjustableColumn2: int, flagadjustableColumn3: int, flagadjustableColumn4: int, flagadjustableColumn5: int, flagadjustableColumn6: int, flagannotation: string, flagattribute: name, flagbackgroundColor: tuple[float, float, float], flagcolumnAlign: tuple[int, string], flagcolumnAlign2: tuple[string, string], flagcolumnAlign3: tuple[string, string, string], flagcolumnAlign4: tuple[string, string, string, string], flagcolumnAlign5: tuple[string, string, string, string, string], flagcolumnAlign6: tuple[string, string, string, string, string, string], flagcolumnAttach: tuple[int, string, int], flagcolumnAttach2: tuple[string, string], flagcolumnAttach3: tuple[string, string, string], flagcolumnAttach4: tuple[string, string, string, string], flagcolumnAttach5: tuple[string, string, string, string, string], flagcolumnAttach6: tuple[string, string, string, string, string, string], flagcolumnOffset2: tuple[int, int], flagcolumnOffset3: tuple[int, int, int], flagcolumnOffset4: tuple[int, int, int, int], flagcolumnOffset5: tuple[int, int, int, int, int], flagcolumnOffset6: tuple[int, int, int, int, int, int], flagcolumnWidth: tuple[int, int], flagcolumnWidth1: int, flagcolumnWidth2: tuple[int, int], flagcolumnWidth3: tuple[int, int, int], flagcolumnWidth4: tuple[int, int, int, int], flagcolumnWidth5: tuple[int, int, int, int, int], flagcolumnWidth6: tuple[int, int, int, int, int, int], flagdefineTemplate: string, flagdocTag: string, flagdragCallback: script, flagdropCallback: script, flagenable: boolean, flagenableBackground: boolean, flagenableKeyboardFocus: boolean, flagenumeratedItem: tuple[int, string], flagexists: boolean, flagfullPathName: boolean, flagheight: int, flaghighlightColor: tuple[float, float, float], flagisObscured: boolean, flaglabel: string, flagmanage: boolean, flagnoBackground: boolean, flagnumberOfPopupMenus: boolean, flagparent: string, flagpopupMenuArray: boolean, flagpreventOverride: boolean, flagrowAttach: tuple[int, string, int], flagstatusBarMessage: string, flaguseTemplate: string, flagvisible: boolean, flagvisibleChangeCommand: script, flagwidth: int) -> string:
    """Synopsis:
---
---
...
def attrFieldGrp(flagadjustableColumn: int, flagadjustableColumn2: int, flagadjustableColumn3: int, flagadjustableColumn4: int, flagadjustableColumn5: int, flagadjustableColumn6: int, flagannotation: string, flagattribute: string, flagbackgroundColor: tuple[float, float, float], flagchangeCommand: script, flagcolumnAlign: tuple[int, string], flagcolumnAlign2: tuple[string, string], flagcolumnAlign3: tuple[string, string, string], flagcolumnAlign4: tuple[string, string, string, string], flagcolumnAlign5: tuple[string, string, string, string, string], flagcolumnAlign6: tuple[string, string, string, string, string, string], flagcolumnAttach: tuple[int, string, int], flagcolumnAttach2: tuple[string, string], flagcolumnAttach3: tuple[string, string, string], flagcolumnAttach4: tuple[string, string, string, string], flagcolumnAttach5: tuple[string, string, string, string, string], flagcolumnAttach6: tuple[string, string, string, string, string, string], flagcolumnOffset2: tuple[int, int], flagcolumnOffset3: tuple[int, int, int], flagcolumnOffset4: tuple[int, int, int, int], flagcolumnOffset5: tuple[int, int, int, int, int], flagcolumnOffset6: tuple[int, int, int, int, int, int], flagcolumnWidth: tuple[int, int], flagcolumnWidth1: int, flagcolumnWidth2: tuple[int, int], flagcolumnWidth3: tuple[int, int, int], flagcolumnWidth4: tuple[int, int, int, int], flagcolumnWidth5: tuple[int, int, int, int, int], flagcolumnWidth6: tuple[int, int, int, int, int, int], flagdefineTemplate: string, flagdocTag: string, flagdragCallback: script, flagdropCallback: script, flagenable: boolean, flagenableBackground: boolean, flagenableKeyboardFocus: boolean, flagexists: boolean, flagextraButton: boolean, flagextraButtonCommand: script, flagextraButtonIcon: string, flagextraLabel: string, flagforceAddMapButton: boolean, flagfullPathName: boolean, flagheight: int, flaghideMapButton: boolean, flaghighlightColor: tuple[float, float, float], flagisObscured: boolean, flaglabel: string, flagmanage: boolean, flagmaxValue: float, flagminValue: float, flagnoBackground: boolean, flagnumberOfFields: int, flagnumberOfPopupMenus: boolean, flagparent: string, flagpopupMenuArray: boolean, flagprecision: int, flagpreventOverride: boolean, flagrowAttach: tuple[int, string, int], flagstatusBarMessage: string, flagstep: float, flaguseTemplate: string, flagvisible: boolean, flagvisibleChangeCommand: script, flagwidth: int) -> string:
    """Synopsis:
---
---
 attrFieldGrp(
...
def attrFieldSliderGrp(flagadjustableColumn: int, flagadjustableColumn2: int, flagadjustableColumn3: int, flagadjustableColumn4: int, flagadjustableColumn5: int, flagadjustableColumn6: int, flagannotation: string, flagattribute: string, flagbackgroundColor: tuple[float, float, float], flagchangeCommand: script, flagcolumnAlign: tuple[int, string], flagcolumnAlign2: tuple[string, string], flagcolumnAlign3: tuple[string, string, string], flagcolumnAlign4: tuple[string, string, string, string], flagcolumnAlign5: tuple[string, string, string, string, string], flagcolumnAlign6: tuple[string, string, string, string, string, string], flagcolumnAttach: tuple[int, string, int], flagcolumnAttach2: tuple[string, string], flagcolumnAttach3: tuple[string, string, string], flagcolumnAttach4: tuple[string, string, string, string], flagcolumnAttach5: tuple[string, string, string, string, string], flagcolumnAttach6: tuple[string, string, string, string, string, string], flagcolumnOffset2: tuple[int, int], flagcolumnOffset3: tuple[int, int, int], flagcolumnOffset4: tuple[int, int, int, int], flagcolumnOffset5: tuple[int, int, int, int, int], flagcolumnOffset6: tuple[int, int, int, int, int, int], flagcolumnWidth: tuple[int, int], flagcolumnWidth1: int, flagcolumnWidth2: tuple[int, int], flagcolumnWidth3: tuple[int, int, int], flagcolumnWidth4: tuple[int, int, int, int], flagcolumnWidth5: tuple[int, int, int, int, int], flagcolumnWidth6: tuple[int, int, int, int, int, int], flagdefineTemplate: string, flagdocTag: string, flagdragCallback: script, flagdropCallback: script, flagenable: boolean, flagenableBackground: boolean, flagenableKeyboardFocus: boolean, flagexists: boolean, flagextraButton: boolean, flagextraButtonCommand: script, flagextraButtonIcon: string, flagfieldMaxValue: float, flagfieldMinValue: float, flagfieldStep: float, flagforceAddMapButton: boolean, flagfullPathName: boolean, flagheight: int, flaghideMapButton: boolean, flaghighlightColor: tuple[float, float, float], flagisObscured: boolean, flaglabel: string, flagmanage: boolean, flagmaxValue: float, flagminValue: float, flagnoBackground: boolean, flagnumberOfPopupMenus: boolean, flagparent: string, flagpopupMenuArray: boolean, flagprecision: int, flagpreventOverride: boolean, flagrowAttach: tuple[int, string, int], flagsliderMaxValue: float, flagsliderMinValue: float, flagsliderStep: float, flagstatusBarMessage: string, flagstep: float, flaguseTemplate: string, flagvertical: boolean, flagvisible: boolean, flagvisibleChangeCommand: script, flagwidth: int) -> string:
    """Synopsis:
---
---
 attrFieldSliderGrp(
...
def attrNavigationControlGrp(flagadjustableColumn: int, flagadjustableColumn2: int, flagadjustableColumn3: int, flagadjustableColumn4: int, flagadjustableColumn5: int, flagadjustableColumn6: int, flagannotation: string, flagattrNavDecision: tuple[name, string], flagattribute: name, flagbackgroundColor: tuple[float, float, float], flagcolumnAlign: tuple[int, string], flagcolumnAlign2: tuple[string, string], flagcolumnAlign3: tuple[string, string, string], flagcolumnAlign4: tuple[string, string, string, string], flagcolumnAlign5: tuple[string, string, string, string, string], flagcolumnAlign6: tuple[string, string, string, string, string, string], flagcolumnAttach: tuple[int, string, int], flagcolumnAttach2: tuple[string, string], flagcolumnAttach3: tuple[string, string, string], flagcolumnAttach4: tuple[string, string, string, string], flagcolumnAttach5: tuple[string, string, string, string, string], flagcolumnAttach6: tuple[string, string, string, string, string, string], flagcolumnOffset2: tuple[int, int], flagcolumnOffset3: tuple[int, int, int], flagcolumnOffset4: tuple[int, int, int, int], flagcolumnOffset5: tuple[int, int, int, int, int], flagcolumnOffset6: tuple[int, int, int, int, int, int], flagcolumnWidth: tuple[int, int], flagcolumnWidth1: int, flagcolumnWidth2: tuple[int, int], flagcolumnWidth3: tuple[int, int, int], flagcolumnWidth4: tuple[int, int, int, int], flagcolumnWidth5: tuple[int, int, int, int, int], flagcolumnWidth6: tuple[int, int, int, int, int, int], flagconnectAttrToDropped: script, flagconnectNodeToDropped: script, flagconnectToExisting: script, flagcreateNew: script, flagdefaultTraversal: script, flagdefineTemplate: string, flagdelete: string, flagdisconnect: script, flagdocTag: string, flagdragCallback: script, flagdropCallback: script, flagenable: boolean, flagenableBackground: boolean, flagenableKeyboardFocus: boolean, flagexists: boolean, flagextraButton: boolean, flagextraButtonCommand: script, flagextraButtonIcon: string, flagfullPathName: boolean, flagheight: int, flaghighlightColor: tuple[float, float, float], flagignore: script, flagignoreNotSupported: boolean, flagisObscured: boolean, flaglabel: string, flagmanage: boolean, flagnoBackground: boolean, flagnoIgnorableMenu: boolean, flagnoKeyableMenu: boolean, flagnumberOfPopupMenus: boolean, flagparent: string, flagpopupMenuArray: boolean, flagpreventOverride: boolean, flagrelatedNodes: script, flagrowAttach: tuple[int, string, int], flagstatusBarMessage: string, flagunignore: script, flaguseTemplate: string, flagvisible: boolean, flagvisibleChangeCommand: script, flagwidth: int) -> string:
    """Synopsis:
---
---
...
def attributeInfo(flagallAttributes: boolean, flagbool: boolean, flagenumerated: boolean, flaghidden: boolean, flaginherited: boolean, flaginternal: boolean, flagleaf: boolean, flaglogicalAnd: boolean, flagmulti: boolean, flagshort: boolean, flagtype: string, flaguserInterface: boolean, flagwritable: boolean) -> list[string]:
    """Synopsis:
---
---
...
def attributeMenu(flagbeginMenu: boolean, flageditor: string, flagfinishMenu: boolean, flaginputs: boolean, flagplug: name, flagregPulldownMenuCommand: string, flagunregPulldownMenuCommand: int) -> string:
    """Synopsis:
---
---
...
def attributeName(flagleaf: boolean, flaglong: boolean, flagnice: boolean, flagshort: boolean) -> string:
    """Synopsis:
---
---
...
def attributeQuery(flagaffectsAppearance: boolean, flagaffectsWorldspace: boolean, flagattributeType: boolean, flagcachedInternally: boolean, flagcategories: boolean, flagchannelBox: boolean, flagconnectable: boolean, flagenum: boolean, flagexists: boolean, flaghidden: boolean, flagindeterminant: boolean, flagindexMatters: boolean, flaginternal: boolean, flaginternalGet: boolean, flaginternalSet: boolean, flagkeyable: boolean, flaglistChildren: boolean, flaglistDefault: boolean, flaglistEnum: boolean, flaglistParent: boolean, flaglistSiblings: boolean, flaglocalizedListEnum: boolean, flaglongName: boolean, flagmaxExists: boolean, flagmaximum: boolean, flagmessage: boolean, flagminExists: boolean, flagminimum: boolean, flagmulti: boolean, flagniceName: boolean, flagnode: name, flagnumberOfChildren: boolean, flagrange: boolean, flagrangeExists: boolean, flagreadable: boolean, flagrenderSource: boolean, flagshortName: boolean, flagsoftMax: boolean, flagsoftMaxExists: boolean, flagsoftMin: boolean, flagsoftMinExists: boolean, flagsoftRange: boolean, flagsoftRangeExists: boolean, flagstorable: boolean, flagtype: string, flagtypeExact: string, flagusedAsColor: boolean, flagusedAsFilename: boolean, flagusesMultiBuilder: boolean, flagworldspace: boolean, flagwritable: boolean) -> boolean | float[]:
    """Synopsis:
---
---
...
def audioTrack(flaginsertTrack: uint, flaglock: boolean, flagmute: boolean, flagnumTracks: uint, flagremoveEmptyTracks: boolean, flagremoveTrack: uint, flagsolo: boolean, flagswapTracks: tuple[uint, uint], flagtitle: string, flagtrack: uint) -> None:
    """Synopsis:
---
---
...
def autoKeyframe(flagaddAttr: name, flagcharacterOption: string, flaglistAttr: boolean, flagnoReset: boolean, flagstate: boolean) -> int:
    """Synopsis:
---
---
...
def autoPlace(flaguseMouse: boolean) -> float[]:
    """Synopsis:
---
---
...
def autoSave(flagdestination: int, flagdestinationFolder: boolean, flagenable: boolean, flagfolder: string, flaginterval: float, flaglimitBackups: boolean, flagmaxBackups: int, flagperform: boolean, flagprompt: boolean) -> None:
    """Synopsis:
---
---
