...
def joint(flagabsolute: boolean, flagangleX: angle, flagangleY: angle, flagangleZ: angle, flagassumePreferredAngles: boolean, flagautomaticLimits: boolean, flagchildren: boolean, flagcomponent: boolean, flagdegreeOfFreedom: string, flagexists: string, flaglimitSwitchX: boolean, flaglimitSwitchY: boolean, flaglimitSwitchZ: boolean, flaglimitX: tuple[angle, angle], flaglimitY: tuple[angle, angle], flaglimitZ: tuple[angle, angle], flagname: string, flagorientJoint: string, flagorientation: tuple[angle, angle, angle], flagposition: tuple[linear, linear, linear], flagradius: float, flagrelative: boolean, flagrotationOrder: string, flagscale: tuple[float, float, float], flagscaleCompensate: boolean, flagscaleOrientation: tuple[angle, angle, angle], flagsecondaryAxisOrient: string, flagsetPreferredAngles: boolean, flagstiffnessX: float, flagstiffnessY: float, flagstiffnessZ: float, flagsymmetry: boolean, flagsymmetryAxis: string, flagzeroScaleOrient: boolean) -> string:
    """Synopsis:
---
---
 joint(
...
def jointCluster(flagaboveBound: float, flagaboveCluster: boolean, flagaboveDropoffType: string, flagaboveValue: float, flagbelowBound: float, flagbelowCluster: boolean, flagbelowDropoffType: string, flagbelowValue: float, flagdeformerTools: boolean, flagjoint: string, flagname: string) -> string:
    """Synopsis:
---
---
 jointCluster(
...
def jointCtx(flagautoJointOrient: string, flagautoPriorityH: boolean, flagcreateIKHandle: boolean, flagdegreeOfFreedomJ: string, flagexists: boolean, flagforceSolverH: boolean, flagimage1: string, flagimage2: string, flagimage3: string, flagjointAutoLimits: boolean, flagjointOrientationJ: tuple[angle, angle, angle], flaglargeBoneLength: float, flaglargeBoneRadius: float, flagpoWeightH: float, flagpriorityH: int, flagscaleCompensateJ: boolean, flagscaleJ: tuple[float, float, float], flagscaleOrientationJ: tuple[angle, angle, angle], flagsecondaryAxisOrient: string, flagsmallBoneLength: float, flagsmallBoneRadius: float, flagsnapHandleH: boolean, flagsolverTypeH: string, flagstickyH: string, flagsymmetry: boolean, flagsymmetryAxis: string, flagvariableBoneSize: boolean, flagweightH: float) -> string:
    """Synopsis:
---
---
 jointCtx(
...
def jointDisplayScale(flagabsolute: boolean, flagikfk: float) -> float:
    """Synopsis:
---
---
 jointDisplayScale(
...
def jointLattice(flagafter: boolean, flagafterReference: boolean, flagbefore: boolean, flagcomponents: boolean, flagcreasing: float, flagdeformerTools: boolean, flagexclusive: string, flagfrontOfChain: boolean, flaggeometry: string, flaggeometryIndices: boolean, flagignoreSelected: boolean, flagincludeHiddenSelections: boolean, flagjoint: string, flaglengthIn: float, flaglengthOut: float, flaglowerBindSkin: string, flaglowerTransform: string, flagname: string, flagparallel: boolean, flagprune: boolean, flagremove: boolean, flagrounding: float, flagselectedComponents: boolean, flagsplit: boolean, flagupperBindSkin: string, flagupperTransform: string, flaguseComponentTags: boolean, flagwidthLeft: float, flagwidthRight: float) -> string:
    """Synopsis:
---
---
 jointLattice(
