...
def vectorize(flagbrowserView: boolean, flagbyFrame: float, flagcamera: string, flagcombineFillsEdges: boolean, flagcurrentFrame: boolean, flagcurveTolerance: float, flagcustomExtension: string, flagdetailLevel: int, flagedgeColor: tuple[int, int, int], flagedgeDetail: boolean, flagedgeStyle: string, flagedgeWeight: float, flagendFrame: float, flagfilenameFormat: string, flagfillStyle: string, flagflashVersion: int, flagframeRate: int, flagheight: int, flaghiddenEdges: boolean, flaghighlightLevel: int, flaghighlights: boolean, flagimageFormat: string, flaglayer: name, flagminEdgeAngle: float, flagoutlinesAtIntersections: boolean, flagoutputFileName: string, flagpixelAspectRatio: float, flagreflectionDepth: int, flagreflections: boolean, flagrenderLayers: boolean, flagrenderOptimization: string, flagrenderView: boolean, flagsecondaryCurveFitting: boolean, flagshadows: boolean, flagshowBackFaces: boolean, flagstartFrame: float, flagsvgAnimation: string, flagsvgCompression: boolean, flagwidth: int) -> None:
    """Synopsis:
---
---
...
def view2dToolCtx(flagalternateContext: boolean, flagboxzoom: boolean, flagdolly: boolean, flagexists: boolean, flaghistory: boolean, flagimage1: string, flagimage2: string, flagimage3: string, flagname: string, flagtoolName: string, flagtrack: boolean) -> string:
    """Synopsis:
---
---
...
def viewCamera(flagmove: name, flagsideView: boolean, flagtopView: boolean) -> None:
    """Synopsis:
---
---
 viewCamera(
...
def viewClipPlane(flagautoClipPlane: boolean, flagfarClipPlane: linear, flagnearClipPlane: linear, flagsurfacesOnly: boolean) -> None:
    """Synopsis:
---
---
 viewClipPlane(
...
def viewFit(flagallObjects: boolean, flaganimate: boolean, flagcenter: boolean, flagfitClipPlanes: boolean, flagfitFactor: float, flagnamespace: string, flagnoChildren: boolean) -> None:
    """Synopsis:
---
---
 viewFit(
...
def viewLookAt(flagposition: tuple[linear, linear, linear]) -> None:
    """Synopsis:
---
---
 viewLookAt(
...
def viewManip(flagbottomLeft: boolean, flagbottomRight: boolean, flagcompassAngle: float, flagdragSnap: boolean, flagdrawCompass: boolean, flagfitToView: boolean, flagfrontParameters: string, flaggoDefault: boolean, flaggoHome: boolean, flaghomeParameters: string, flaglevelCamera: boolean, flagminOpacity: float, flagnamespace: string, flagpostCommand: string, flagpreCommand: string, flagpreserveSceneUp: boolean, flagresetFront: boolean, flagresetHome: boolean, flagrestoreCenter: boolean, flagselectionLockParameters: string, flagsetFront: boolean, flagsetHome: boolean, flagsize: string, flagtoggleSelectionLock: boolean, flagtopLeft: boolean, flagtopRight: boolean, flagvisible: boolean, flagzoomToFitScene: boolean) -> None:
    """Synopsis:
---
---
...
def viewPlace(flaganimate: boolean, flageyePoint: tuple[linear, linear, linear], flagfieldOfView: angle, flaglookAt: tuple[linear, linear, linear], flagortho: boolean, flagperspective: boolean, flagupDirection: tuple[linear, linear, linear], flagviewDirection: tuple[linear, linear, linear]) -> None:
    """Synopsis:
---
---
 viewPlace(
...
def viewSet(flaganimate: boolean, flagback: boolean, flagbottom: boolean, flagfit: boolean, flagfitFactor: float, flagfront: boolean, flaghome: boolean, flagkeepRenderSettings: boolean, flagleftSide: boolean, flagnamespace: string, flagnextView: boolean, flagpersp: boolean, flagpreviousView: boolean, flagrightSide: boolean, flagside: boolean, flagtop: boolean, flagviewNegativeX: boolean, flagviewNegativeY: boolean, flagviewNegativeZ: boolean, flagviewX: boolean, flagviewY: boolean, flagviewZ: boolean) -> None:
    """Synopsis:
---
---
 viewSet(
...
def visor(flagaddFolder: boolean, flagaddNodes: string, flagallowPanningInX: boolean, flagallowPanningInY: boolean, flagallowZooming: boolean, flagcommand: string, flagdeleteFolder: string, flageditFolder: string, flagfolderList: string, flagmenu: string, flagname: string, flagnodeType: string, flagopenDirectories: boolean, flagopenFolder: boolean, flagparent: string, flagpath: string, flagpopupMenuScript: string, flagrebuild: boolean, flagrefreshAllSwatches: boolean, flagrefreshSelectedSwatches: boolean, flagrefreshSwatch: string, flagreset: boolean, flagrestrictPanAndZoom: boolean, flagsaveSwatches: boolean, flagscrollBar: string, flagscrollPercent: float, flagselectedGadgets: string, flagshowDividers: boolean, flagshowFiles: boolean, flagshowFolders: boolean, flagshowNodes: boolean, flagstateString: boolean, flagstyle: string, flagtransform: string, flagtype: string) -> string:
    """Synopsis:
---
---
...
def volumeAxis(flagalongAxis: float, flagaroundAxis: float, flagattenuation: float, flagawayFromAxis: float, flagawayFromCenter: float, flagdetailTurbulence: float, flagdirectionX: float, flagdirectionY: float, flagdirectionZ: float, flagdirectionalSpeed: float, flaginvertAttenuation: boolean, flagmagnitude: float, flagmaxDistance: linear, flagname: string, flagperVertex: boolean, flagposition: tuple[linear, linear, linear], flagtorusSectionRadius: linear, flagturbulence: float, flagturbulenceFrequencyX: float, flagturbulenceFrequencyY: float, flagturbulenceFrequencyZ: float, flagturbulenceOffsetX: float, flagturbulenceOffsetY: float, flagturbulenceOffsetZ: float, flagturbulenceSpeed: float, flagvolumeExclusion: boolean, flagvolumeOffset: tuple[linear, linear, linear], flagvolumeShape: string, flagvolumeSweep: angle) -> string:
    """Synopsis:
---
---
 volumeAxis(
...
def volumeBind(flaginfluence: string, flagname: string) -> list[string]:
    """Synopsis:
---
---
 volumeBind(
